<#
.SYNOPSIS
Exports the GridForge Unity package variants in batch mode.

.DESCRIPTION
Launches Unity without opening the editor UI and executes
GridForgeUnityPackageExporter.ExportUnityPackagesBatchMode. The exporter
...
$additionalArguments = @(
    "-GridForgeUnityPackageOutputPath",
    $OutputPath
)

Invoke-GridForgeUnityBatchMethod `
    -ExecuteMethod "GridForge.Build.Editor.GridForgeUnityPackageExporter.ExportUnityPackagesBatchMode" `
    -UnityPath $UnityPath `
    -ProjectPath $ProjectPath `
    -LogFile $LogFile `
    -AdditionalArguments $additionalArguments `
    -PathArgumentNames @("-GridForgeUnityPackageOutputPath") `
    -WhatIf:$WhatIf
